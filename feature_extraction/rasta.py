...
def rastaplp(x, fs=16000, win_time=0.025, hop_time=0.010, dorasta=True, modelorder=8):   # win_time=0.040, hop_time=0.020
...
def log_mel_fb(samples, fs, feature_dimension,delta):
    ...
    # temporal = librosa.feature.tempogram(y = samples, sr = fs,  hop_length=step, win_length=window_size)
    ...
    if delta is True:
        delta1 = librosa.feature.delta(spectogram_log, order=1, axis=- 1)
        delta2 = librosa.feature.delta(spectogram_log, order=2, axis=- 1)
        spectogram_log = np.concatenate((spectogram_log, delta1, delta2), axis=1)
